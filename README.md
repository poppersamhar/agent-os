...
人类员工通过自然语言与 AI Agent 协作，Agent 自动拆解任务、调度 Sub-Agent 执行、产出结果并沉淀知识。
...
| **Member 首页** | 个人数据看板（Token 消耗、活跃项目、任务成功率）+ HubAgent 对话区 |
| **Admin 首页** | 全组织数据看板 + OrgAgent 对话区 + 组织管理导航 |
| **项目创建** | Sidebar `[+]` 或 HubAgent 对话触发，项目为创建者私有 |
| **项目页** | 任务列表 + WorkAgent 对话区 + 文件 Tab（上下文 + 产物）+ 知识图谱 |
| **任务创建** | 项目内 / 独立任务两种模式 |
| **任务执行页** | 聊天式执行流，8 种消息类型（member/workagent/question/plan/subagent/completion/system/permission）|
| **右侧面板** | 任务树 / 项目树 / 知识图谱（知识图谱 + 上下文演化）三 Tab |
...
- **首页级**：点击项目 context.md 可 `@引用` 到 HubAgent 输入框

## 产品架构

```
┌─────────────────────────────────────────────┐
│  第一层：效能管理（OrgAgent / HubAgent）       │
│  全局视角：跨项目分析、风险监控、审批汇集       │
├─────────────────────────────────────────────┤
│  第二层：管理智能体（WorkAgent）               │
│  任务编排：需求解析 → 方案规划 → Sub-Agent 调度 │
├─────────────────────────────────────────────┤
│  第三层：执行层（Sub-Agent / Skill / Tool）    │
│  原子执行：数据采集、代码生成、报告撰写...     │
└─────────────────────────────────────────────┘
```
