...
| 层级 | 名称 | 定位 |
|------|------|------|
| 第一层 | 企业效能管理智能体（OrgAgent） | 全局视角：跨项目分析、风险监控、审批汇集、效能管理 |
| 第二层 | 管理智能体（WorkAgent） | 动态编排：计划、执行、审查，调度数字员工完成任务 |
| 第三层 | 数字员工（Sub-Agent / Skill / Tool） | 具体执行：调用 Skill、操作工具、完成原子任务 |
...
|-------------|---------------|-------------|
| **第一层：企业效能管理智能体（OrgAgent）** | 基于 Session Log 的全局观测与治理层 | 如果"群聊"只是内存中的消息队列，第一层很难做真正的审计与风险分析 |
| **第二层：管理智能体（管理智能体（WorkAgent））** | **Harness** | 管理智能体（WorkAgent）如果是"运行在某个容器里的 Agent"，就可能变成了 Pet；需要与执行环境解耦 |
| **第三层：数字员工（Sub-Agent / Skill / Tool）** | **Hands / Sandbox / Tools** | 需要统一为 `execute(name, input)` 接口，且不应接触凭证 |

**一句话总结：AgentOS 的"群聊"概念应该升级为 Anthropic 的 Session 抽象，管理智能体（WorkAgent）应该升级为 Harness 抽象，Skill 层应该升级为 Hands 抽象。**
...
- **群聊即 Session，Session 即 Append-only Log**
  - 每个项目/Session 对应一个持久化的事件流，记录：谁说了什么、管理智能体（WorkAgent）下了什么计划、@了谁、Skill 返回了什么结果、是否报错、是否重试。

- **Harness（管理智能体（WorkAgent））通过查询接口读取历史，而非直接持有历史**
  - 管理智能体（WorkAgent） Agent 不直接把"群聊 500 条消息"塞进 prompt。而是调用 `session.getEvents(start, end)` 按需切片，再由 Harness 层做 context engineering（摘要、过滤、组织）。
...
### 4.2 Harness 层：管理智能体（WorkAgent） Agent 要与 Skill 解耦

**现状风险：** 如果管理智能体（WorkAgent） Agent 和 Skill Agent 跑在同一个运行时/容器里，Skill 崩溃可能导致管理智能体（WorkAgent）也挂掉；或者管理智能体（WorkAgent）必须等 Skill 环境初始化完才能开始推理（TTFT 高）。

**借鉴做法：**

- **管理智能体（WorkAgent）（Harness）是"无状态"的**
  - 管理智能体（WorkAgent）只负责：读取 Session → 组织上下文 → 调用 LLM → 拿到 tool call → 通过统一接口调用 Skill → 把结果写回 Session。
  - 如果管理智能体（WorkAgent）进程挂了，新拉起一个实例，读取同一个 Session Id，从最后一条事件恢复。

- **Skill 调用统一为 `execute(skillName, input) → string`**
  - 管理智能体（WorkAgent）不直接 import/调用 Skill 代码，而是通过 RPC/MCP/HTTP 调用。对管理智能体（WorkAgent）来说，第三方 Agent、本地 Skill、知识工程服务、数据分析沙箱，都是同一个接口。

- **Lazy Provisioning（延迟初始化）**
  - 不是一建群就把所有 Skill 的容器都拉起来，而是管理智能体（WorkAgent）决定调用某个 Skill 时，才触发 `provision()`。这样空群聊或纯讨论型 Session 的启动延迟极低。
...
- **失败即替换**
  - Skill 执行失败（超时、报错、返回格式不对），管理智能体（WorkAgent） Harness 像 Anthropic 那样把错误当 tool-call error 捕获，自行决定重试、降级或换另一个 Skill。

- **Many Hands 支持**
  - 一个管理智能体（WorkAgent）可以同时持有多个 Skill Handle。比如一个数据分析任务，管理智能体（WorkAgent）同时把 SQL 生成交给 Skill A、把可视化交给 Skill B、把数据校验交给 Skill C，自己等结果后整合。
...
- **凭证绝不进入 Sandbox / Skill 环境**
  - 第三层 Skill（尤其是第三方 Agent 或代码执行沙箱）不应该拿到数据库密码、API Key。采用 Proxy/Vault 模式：
    - 管理智能体（WorkAgent） Harness 也不持有凭证。
...
### 4.5 上下文工程外化：管理智能体（WorkAgent）不是"记性好"，而是"会查档案"
...
- **管理智能体（WorkAgent）的"记忆"不在 prompt 里，而在 Session Log 里**
  - 跨 Session 的上下文，不是通过"给管理智能体（WorkAgent）塞一个超长 summary"实现的，而是管理智能体（WorkAgent）在新的 Session 中通过查询历史 Session 的 Event Log 来重建上下文。

- **Context Engineering 是 Harness 层的实现细节**
  - 未来模型能力变了（比如上下文窗口从 200k 变 2M），不需要改管理智能体（WorkAgent）的业务逻辑，只需要升级 Harness 层怎么组织 `getEvents()` 的结果。这正符合产品"目标不是让模型回答更好，而是让企业能编排任务流"的定位。
...
└─────────────────────────────────────────┘
         ↑读取/恢复          ↓写入
┌─────────────────────────────────────────┐
│  第二层：管理智能体 / Harness（管理智能体（WorkAgent））      │
...
| **中间/右侧聊天** | Harness 交互层 | 人类员工与管理智能体（WorkAgent）/Agent/Skill 的群聊式任务协作 |
...
**项目页（群聊）**：
- 右侧聊天框 ↔ 与第二层"管理智能体（WorkAgent） Agent"及第三层 Agent/Skill 的群聊
- 左侧可视化 ↔ 任务执行流、知识图谱、审计日志
- 体现"管理智能体（WorkAgent）拉人进群、@下派任务、Many Hands"
...
4. **上下文管理 Harness 化**：将管理智能体（WorkAgent） Agent 的 prompt 组装逻辑从硬编码改为从 Session Log 动态读取，支持 compaction、trimming 等策略的可插拔。
